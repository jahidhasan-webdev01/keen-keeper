...
const Friends = async () => {
    const friendsData = await friendPromis();
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mt-5">
                {
                    friendsData.map((data, index) =>
                        <div
                            key={index}
                            className="border flex flex-col justify-center items-center bg-white py-10 gap-4 px-2"
                        >
                            ...
                            <div className="flex flex-wrap gap-2">
                                {
                                    data.tags.map((tag, index) =>
                                        <p
                                            key={index}
                                            className="bg-green-200 w-fit px-2 py-1 rounded-full text-green-900 font-medium"
                                        >
                                            {tag.toUpperCase()}
                                        </p>
                                    )
                            ...
                            <p className={`${data.status === "overdue"
                                ...
                                } text-white px-2 py-1 rounded-full`}>{data.status}</p>
                        </div>)
                }
            </div>
        </div>
